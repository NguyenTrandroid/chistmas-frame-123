...
import stickertest.com.firebase.bimbim.collagedemo.App;
import stickertest.com.firebase.bimbim.collagedemo.blur.StackBlurManager;
    ...
    public static void loadImageWithGlide(Context context, ImageView imageView, String uri) {
        if (uri != null && uri.length() > 1) {
            if (uri.startsWith("http://") || uri.startsWith("https://")) {
                ...
                Glide.with(context).load(Uri.parse("file:///android_asset/".concat(file)))
                       // .crossFade()
                      // .signature(String.valueOf(System.currentTimeMillis()))
                   .into(imageView);
//                Glide.with(context).load(Uri.fromFile(new File(SupportUtils.getRootDirPath(App.getAppContext())+"/"+file)))
//                        //.crossFade()
